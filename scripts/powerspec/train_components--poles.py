...
parser.add_argument(
    "--cosmo_params",
    help="cosmological parameters included in the emulation.",
    nargs="+",
    type=str,
    required=True,
)
...
if not kernels.shape == (Nsamp, 37, 119):
    raise (
        ValueError(
            f"The kernel array has the wrong shape: {kernels.shape}. Should be: {(Nsamp, 37, 119)}."
        )
    )
...
pole_col_dict = {
    0: np.arange(16),
    2: np.arange(17, 28),
    4: np.append(np.arange(28, 34), 16),
...
for k in pole_col_dict:
    ...
    # Save parameters of scaler.
    np.save(
        cache_path + f"P{k}/scalers/yscaler_min_diff",
    ...
    reduce_lr = ReduceLROnPlateau(
        monitor="loss", factor=0.1, patience=10, min_lr=0.0001, mode="min", cooldown=5
    )
