...
np.save(
    cache_path + "/shot/scalers/xscaler_min_diff",
...
if not kernels.shape[0] == Nsamp or not kernels.shape[1] == 8:
    raise (
        ValueError(
            f"The kernel array has the wrong shape: {kernels.shape}. Should be: ({Nsamp}, 8, k lenght)."
        )
    )
...
# Save parameters of scaler.
np.save(
    cache_path + "/shot/scalers/" + component_name + "/yscaler_min_diff",
