...
if not kernels.shape[0] == Nsamp or not kernels.shape[1] == 52:
    raise(ValueError(f"The kernel array has the wrong shape: {kernels.shape}. Should be: ({Nsamp}, 52, k lenght)."))
...
for i in range(7):
    ...
    ind = []
    for j in group_list: ind.append(helper.bispec_bias_sorted_kernels.index(j))
    ...
    # Save parameters of scaler.
    np.save(cache_path+f"/scalers/group_{i}/yscaler_min_diff",
