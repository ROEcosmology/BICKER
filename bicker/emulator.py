...
kbins = np.arange(0.005, 0.2025, 0.0025)
...
class component_emulator:
    ...
    def __init__(self, group, multipole, kbins, cache_path):
        ...
        if group != 'shot':
            ...
            components_path = (
                cache_path + "bispec/B{l}/shot/".format(l=multipole) + "components/"
            )
            scalers_path = (
                cache_path + "bispec/B{l}/shot/".format(l=multipole) + "scalers/"
            )
            group_id = "group_0"
    ...
    def emu_predict(self, X, split=True):
        ...
        preds = self.scalers[1].inverse_transform(self.model(X_prime))
...
class bispectrum:
    ...
    def emu_predict(self, cosmo, b1=None, b2=None, bG2=None, c1=None, c2=None):
        ...
        bispec_pred = helper_funcs.combine_kernels(
            kernel_group_preds, b1=b1, b2=b2, bG2=bG2, c1=c1, c2=c2
        )
...
class power:
    ...
    def __init__(self, multipole, kbins, cache_path):
        ...
        for i in [multipole, "extra"]:
            self.models.append(
                load_model(cache_path + f"powerspec/P{i}/components/member_0")
            )
            ...
            ymin_diff = np.load(
                cache_path + f"powerspec/P{i}/scalers/yscaler_min_diff.npy"
            )
    ...
    def emu_predict(self, cosmo, bias):
        ...
        preds = self.scalers[0][1].inverse_transform(self.models[0](X_prime))
        preds = preds.reshape(
            cosmo.shape[0],
            int(preds.shape[1] / self.kbins.shape[0]),
            self.kbins.shape[0],
        )

        preds_ext = self.scalers[1][1].inverse_transform(self.models[1](X_prime))
        preds_ext = preds_ext.reshape(
            cosmo.shape[0],
            int(preds_ext.shape[1] / self.kbins.shape[0]),
            self.kbins.shape[0],
        )
