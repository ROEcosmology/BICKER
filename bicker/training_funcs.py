...
Functions origonally written for the matryoshka package https://github.com/JDonaldM/Matryoshka
"""
...
class Resampler:
    ...
    def __init__(
        self, simulation_samples=None, parameter_ranges=None, use_latent_space=False
    ):
        ...
        if (simulation_samples is None) and (parameter_ranges is None):
            raise ValueError(
                "Please provide either simulation samples or parameter ranges."
            )
            ...
            latent_samples = np.matmul(
                np.linalg.inv(self.L), (simulation_samples - self.mean).T
            ).T
    ...
    def new_samples(self, nsamps, LH=True, buffer=None):
        ...
        if (LH is False) and (self.use_latent_space is False):
            return np.random.uniform(
                self.min, self.max, size=(nsamps, self.min.shape[0])
            )
